...
from causal_discovery.enco_ns import ENCONS
...
def get_basic_parser():
    ...
    parser.add_argument(
        "--num_epochs", type=int, default=30, help="Number of epochs to run ENCO for."
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for the experiments."
    )
    parser.add_argument(
        "--cluster",
        action="store_true",
...
def test_graph(graph, args, checkpoint_dir, file_id):
    ...
    discovery_module.discover_graph(
        num_epochs=args.num_epochs, stop_early=args.stop_early
    )
    duration = int(time.time() - start_time)
    print(
        "-> Finished training in %ih %imin %is"
    ...
    if graph.num_vars < 100:
        ...
        with open(
            os.path.join(checkpoint_dir, "metrics_acyclic_%s.json" % file_id), "w"
        ) as f:
            json.dump(metrics_acyclic, f, indent=4)
        print("-" * 50 + "\nFinal metrics (acyclic):")
        discovery_module.print_graph_statistics(m=metrics_acyclic)
    with open(
        os.path.join(checkpoint_dir, "metrics_full_log_%s.json" % file_id), "w"
    ) as f:
    ...
    np.save(
        os.path.join(checkpoint_dir, "binary_matrix_%s.npy" % file_id),
        binary_matrix.astype(bool),
    )
    if graph.num_vars < 100:
        acyclic_matrix = discovery_module.get_acyclic_adjmatrix().detach().numpy()
        np.save(
            os.path.join(checkpoint_dir, "binary_acyclic_matrix_%s.npy" % file_id),
    ...
    if graph.num_vars < 40:
        ...
        visualize_graph(
            ...
            filename=os.path.join(
                checkpoint_dir, "graph_%s_prediction.pdf" % (file_id)
            ),
    ...
    torch.save(state_dict, os.path.join(checkpoint_dir, "state_dict_%s.tar" % file_id))


def test_graph_ns(graph, args, checkpoint_dir, file_id):
    """
    Runs ENCO on a given graph for structure learning.
    ...
    print(discovery_module.get_predicted_ordering())
