...
class NeuralSorting(object):
    ...
    def perform_update_step(self, gamma, var_idx=-1):
        """
    ...
    def get_monte_carlo_samples(self, gamma, num_batches, num_graphs, batch_size, var_idx=-1):
    ...
    def gumbel_softmax(self, n_samples, scores, tau=1.0, beta=1.0):
        ...
        with torch.enable_grad():
            log_s_perturb = torch.log(scores.unsqueeze(0)) + sample_gumbel([n_samples, 1, n_dim, 1], beta=beta).to(scores.device)
            log_s_perturb = log_s_perturb.view(-1, n_dim, 1)
            P_hat = self.neural_sort(log_s_perturb, tau=tau)
            P_hat = P_hat.view(n_samples, -1, n_dim, n_dim)
    ...
    @torch.no_grad()
    def get_MC_samples(self, gamma, num_batches, num_graphs, batch_size,
