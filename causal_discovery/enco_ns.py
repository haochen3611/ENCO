...
from causal_discovery.dist_fitting_ns import DistributionFittingNS
...
class ENCONS(object):
    ...
    def __init__(
        ...
        if graph.is_categorical:
            ...
            model = create_model(
                num_vars=self.num_vars, num_categs=num_categs, hidden_dims=hidden_dims
            )
        else:
    ...
    def init_graph_params(self, num_vars, lr_gamma, betas_gamma):
        """
    ...
    def graph_fitting_step(self, tau=0.1, beta=1.0):
        ...
        for _ in t:
            ...
            loss, var_idx = self.graph_fitting_module.perform_update_step(
                gamma=self.gamma,
                tau=tau,
                beta=beta,
            )
